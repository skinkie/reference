...
con_orig = sqlite3.connect("/home/netex/delijn-netex.duckdb")
con_target = sqlite3.connect("/home/netex/delijn-nl.duckdb")




codespace_ref_or_codespace = GeneratorTester(load_generator(con_orig, Codespace))
data_source = load_local(con_orig, DataSource)
...
transfer = GeneratorTester(chain(load_generator(con_target, Connection), load_generator(con_target, SiteConnection),
                                 load_generator(con_target, DefaultConnection)))
...
publication_delivery = PublicationDelivery(
    ...
    data_objects=DataObjectsRelStructure(choice=[
        CompositeFrame(
            id="CompositeFrame", version=version,
            type_of_frame_ref=TypeOfFrameRef(ref='BISON:TypeOfFrame:NL_TT_BASELINE', version_ref='9.3.0'),
            frame_defaults=VersionFrameDefaultsStructure(default_location_system="EPSG:28992",
            ...
            frames=FramesRelStructure(
                common_frame=[
                    ResourceFrame(
                        id="RESOURCE", version=version,
                        type_of_frame_ref=TypeOfFrameRef(ref='BISON:TypeOfFrame:NL_TT_RESOURCE', version_ref='9.3.0'),
                    ...
                    ServiceFrame(
                        ...
                        type_of_frame_ref=TypeOfFrameRef(ref='BISON:TypeOfFrame:NL_TT_SERVICE', version_ref='9.3.0'),
                        ...
                        #     route_point=route_point.generator()) if route_point.has_value() else None,
                        # route_links=RouteLinksInFrameRelStructure(
                        #     route_link=route_link.generator()) if route_link.has_value() else None,
                        ...
                        #     stop_assignment=stop_assignment.generator()) if stop_assignment.has_value() else None,
                        # notices=NoticesInFrameRelStructure(notice=notice.generator()) if notice.has_value() else None,
                        # notice_assignments=NoticeAssignmentsInFrameRelStructure(
                        #     notice_assignment=notice_assignment.generator()) if notice_assignment.has_value() else None,
                    ),
                    TimetableFrame(
                        id="TIMETABLE", version=version,
                        type_of_frame_ref=TypeOfFrameRef(ref='BISON:TypeOfFrame:NL_TT_TIMETABLE', version_ref='9.3.0'),
                    ...
                    #         day_type=day_type.generator()) if day_type.has_value() else None,
                    #     service_calendar=list(service_calendar.generator())[
                    #         0] if service_calendar.has_value() else None,  # Warning; we must handle multiple stuff
                    # ),
                ]
            )
        )
