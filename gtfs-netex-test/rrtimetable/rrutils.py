from io import RawIOBase, IOBase, RawIOBase
...
def writeshort(out: RawIOBase,x) :
    """Write x as packed binary data to file
    :param out: output file-pointer
...
def writesignedbyte(out: RawIOBase,x) :
    """Write x as packed binary data to file
    :param out: output file-pointer
...
def write_2ushort(out: RawIOBase,x, y) :
...
def write2floats(out: RawIOBase,x, y) :
    """Write coordinate (x,y) as packed binary data to file
    :param out: output file-pointer
...
def align(out: RawIOBase, width=4):
...
def tell(out: RawIOBase) :
    ...
    pos = out.tell()
    if pos > 1024 * 1024 :
        text = f'{pos / 1024.0 / 1024.0:0.2f} MB'
    else :
        text = f'{pos / 1024.0::0.2f} kB'
    print(f"  at position {pos} in output [{text}]")
    return pos

def write_text_comment(out: RawIOBase, string: str):
...
def write_string_table(out: RawIOBase,strings):
