    ...
    async with aiohttp.ClientSession() as session:
        while True:
            ...
            for sender_uri in cursor.fetchall():
                daten_bereit_anfrage = DatenBereitAnfrage(sender=SENDER_ID, zst=XmlDateTime.now())
...
async def aus_datenabrufen(request):
    ...
    if daten_abrufen_anfrage.sender == request.match_info['sender']:
        ...
        if uri is not None:
            antwort = DatenAbrufenAntwort(bestaetigung=BestaetigungType(fehlernummer="0", zst=XmlDateTime.now(), ergebnis=ErgebnisType.OK), choice=await aus_nachrichten(sender=daten_abrufen_anfrage.sender))
        else:
    ...
    if abo_anfrage.sender == request.match_info['sender']:
        ...
        if uri:
            if isinstance(abo_anfrage.choice, bool):
                ...
                antwort = AboAntwort(bestaetigung=BestaetigungType(fehlernummer="0", ergebnis=ErgebnisType.OK, zst=XmlDateTime.now()))
                ...
                antwort = AboAntwort(bestaetigung=BestaetigungType(fehlernummer="0", fehlertext=f"Adding/Replacing abo_id(s) {ids} for {abo_anfrage.sender}.", ergebnis=ErgebnisType.OK, zst=XmlDateTime.now()))
